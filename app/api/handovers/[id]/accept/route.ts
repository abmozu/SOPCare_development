...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { id } = await context.params; const payload = await request.json().catch(() => ({})) as { action?: string }; const db = await ensureDatabase();
    ...
    if (payload.action === "decline") {
      ...
      await db.batch([
        db.prepare("UPDATE clinical_handovers SET status = 'Declined', accepted_at = ? WHERE id = ?").bind(now, id),
      ...
      await writeAudit(actor.id, "CARE_HANDOVER_DECLINED", "injury_episode", handover.injuryId, "Care handover declined");
