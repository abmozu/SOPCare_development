...
export async function POST(request: Request) {
  ...
  try {
    ...
    let practitioner = await db.prepare(`SELECT pp.id
    ...
    if (!practitioner) {
      ...
      await db.prepare(`INSERT INTO practitioner_profiles (id, user_id, specialty, credentials)
        VALUES (?, ?, ?, ?)`).bind(practitionerId, user.id, actor.specialty, "SOPCare practitioner").run();
      practitioner = { id: practitionerId };
    }

    const encounterType = actor.specialty;
    const clinicCity = actor.clinicCity;
    ...
    await db.prepare(`INSERT INTO encounters (id, athlete_id, practitioner_id, encounter_date, encounter_type, clinic_city, clinic_type, clinic_location, reason, subjective, objective, assessment, plan, diagnosis, visibility, follow_up_date)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .bind(id, athleteId, practitioner.id, now, encounterType, clinicCity, "", "", reason, "", "", "", cleanRichHistory(payload.plan), diagnosis, visibility, null)
      .run();
