...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (next) {
      ...
      const completionSummary = `Rehabilitation completed. ${decisionNote}`;
      ...
      statements.push(db.prepare("UPDATE injury_episodes SET stage = 'Closed', participation_status = 'Available', next_action = 'Rehabilitation completed', closure_summary = ?, closed_at = ?, updated_at = ? WHERE id = ?").bind(completionSummary, now, now, plan.injuryId));
      statements.push(db.prepare("INSERT INTO injury_status_history (id, injury_id, from_stage, to_stage, note, changed_by, created_at) VALUES (?, ?, ?, 'Closed', ?, ?, ?)")
        .bind(crypto.randomUUID(), plan.injuryId, plan.injuryStage, completionSummary, actor.id, now));
    }
