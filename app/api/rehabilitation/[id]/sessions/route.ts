...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const phase = await db.prepare("SELECT id FROM rehabilitation_phases WHERE plan_id = ? AND phase_number = ?")
      .bind(id, plan.currentPhase).first<{ id: string }>();
    ...
    const overallProgress = phaseProgress === null ? null : Math.round((((plan.currentPhase - 1) * 100) + phaseProgress) / Math.max(1, phaseCount?.count ?? 1));
    ...
    if (status === "Completed" && phaseProgress !== null && overallProgress !== null) {
