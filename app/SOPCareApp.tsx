...
type ClinicalTask = { id: string; taskType: string; title: string; detail: string; injuryId: string | null; planId: string | null; handoverId: string | null; createdAt: string };
...
type Bootstrap = {
  ...
  athletes: Athlete[]; encounters: Encounter[]; injuries: Injury[]; injuryHistory: InjuryHistory[]; rehabilitationPlans: RehabilitationPlan[]; rehabilitationPhases: RehabilitationPhase[]; rehabilitationExercises: RehabilitationExercise[]; rehabilitationSessions: RehabilitationSession[]; practitioners: Practitioner[]; activities: Activity[]; tasks: ClinicalTask[];
...
export default function SOPCareApp({ identity, onSwitchWorkspace, onLogout }: { identity: PortalUser; onSwitchWorkspace?: () => void; onLogout: () => void }) {
  ...
  const [modal, setModal] = useState<null | "athlete" | "encounter" | "edit" | "care" | "injury" | "injuryStage" | "linkEncounter" | "injuryAssociation" | "rehabilitation" | "rehabSession" | "rehabAdvance" | "clinicalSafety" | "practitionerProfile" | "handover" | "taskHandover" | "returnToPlay">(null);
  const [activeTask, setActiveTask] = useState<ClinicalTask | null>(null);
  ...
  async function handleTask(task: ClinicalTask) {
    if (task.taskType === "handover" && task.handoverId) {
      setActiveTask(task); setModal("taskHandover");
      return;
    }
    setActiveTask(task); setModal("returnToPlay");
  }

  async function submitHandover(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!selectedInjuryId) return; await apiAction(`/api/injuries/${selectedInjuryId}/handover`, { method: "POST", body: JSON.stringify(Object.fromEntries(new FormData(event.currentTarget).entries())) }, "Care handover sent"); }
  async function decideHandover(action: "accept" | "decline") { if (!activeTask?.handoverId) return; await apiAction(`/api/handovers/${activeTask.handoverId}/accept`, { method: "POST", body: JSON.stringify({ action }) }, action === "accept" ? "Care handover accepted" : "Care handover declined"); setActiveTask(null); }
  async function submitReturnToPlay(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!activeTask?.injuryId) return; await apiAction(`/api/injuries/${activeTask.injuryId}/return-to-play`, { method: "POST", body: JSON.stringify(Object.fromEntries(new FormData(event.currentTarget).entries())) }, "Return-to-play decision recorded"); setActiveTask(null); }
    ...
    <div className="app-shell">
      ...
      {modal && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setModal(null); }}>
        <section className={`modal ${modal === "encounter" || modal === "injury" || modal === "rehabilitation" ? "modal-wide" : ""}`} role="dialog" aria-modal="true" aria-labelledby="modal-title">
          ...
          {modal === "handover" && selectedInjury && <HandoverForm injury={selectedInjury} practitioners={data.practitioners.filter((person) => person.userId !== data.actor.id)} onSubmit={submitHandover} busy={busy} />}
          {modal === "taskHandover" && activeTask && <TaskHandoverForm task={activeTask} busy={busy} onAccept={() => decideHandover("accept")} onDecline={() => decideHandover("decline")} />}
          {modal === "returnToPlay" && activeTask && <ReturnToPlayForm task={activeTask} onSubmit={submitReturnToPlay} busy={busy} />}
        </section>
      </div>}
...
function InjuryDetailView({ injury, rehabilitationPlan, history, encounters, expandedEncounters, onExpandedEncountersChange, backLabel, onBack, onAthlete, onEncounter, onStage, onHandover, onRehabilitation, onCreateRehabilitation }: { injury: Injury; rehabilitationPlan?: RehabilitationPlan; history: InjuryHistory[]; encounters: Encounter[]; expandedEncounters: string[]; onExpandedEncountersChange: (ids: string[]) => void; backLabel: string; onBack: () => void; onAthlete: (id: string) => void; onEncounter: (encounter: Encounter) => void; onStage: () => void; onHandover: () => void; onRehabilitation: (id: string) => void; onCreateRehabilitation: () => void }) {
...
function RehabilitationDetailView({ plan, phases, exercises, sessions, onBack, onInjury, onAthlete, onSession, onHandover, onAdvance }: { plan: RehabilitationPlan; phases: RehabilitationPhase[]; exercises: RehabilitationExercise[]; sessions: RehabilitationSession[]; onBack: () => void; onInjury: (id: string) => void; onAthlete: (id: string) => void; onSession: () => void; onHandover: () => void; onAdvance: () => void }) {
...
function EncounterForm({ actor, athlete, onSubmit, busy }: { actor: Bootstrap["actor"]; athlete: Athlete; onSubmit: (e: FormEvent<HTMLFormElement>) => void; busy: boolean }) {
  return <form onSubmit={onSubmit}><ModalHeading kicker="Clinical encounter" title="New encounter" text={`Record a new clinical encounter for ${fullName(athlete)}.`} /><input type="hidden" name="athleteId" value={athlete.id} /><div className="form-grid"><label className="span-2">Reason for visit*<input name="reason" required autoFocus placeholder="Concise clinical reason" /></label><label className="span-2 history-field">History<RichHistoryInput /></label><label className="span-2">Diagnosis*<textarea name="diagnosis" rows={3} required placeholder="Write the clinical diagnosis in plain language — no ICD-10 code required" /></label><label className="span-2">Visibility<select name="visibility"><option>Care team</option><option>Restricted</option></select></label></div><div className="modal-actions"><button className="button primary" type="submit" disabled={busy}>{busy ? "Saving…" : "Save encounter"}</button></div></form>;
}
...
function RehabilitationForm({ injuries, activePlanInjuryIds, selectedInjuryId, onSubmit, busy }: { injuries: Injury[]; activePlanInjuryIds: string[]; selectedInjuryId?: string; onSubmit: (e: FormEvent<HTMLFormElement>) => void; busy: boolean }) {
  ...
  return <form onSubmit={onSubmit}><ModalHeading kicker="Rehabilitation pathway" title="Create rehabilitation plan" text="Define the goal, review cadence, and four-stage criteria-led pathway for an open injury episode." /><div className="form-grid"><label className="span-2">Injury episode*<select name="injuryId" required defaultValue={selectedValue}><option value="">Select open injury</option>{availableInjuries.map((injury) => <option key={injury.id} value={injury.id}>{injury.athleteName} · {injury.title}</option>)}</select></label><label className="span-2">Plan title*<input name="title" required placeholder="e.g. Hamstring return-to-speed pathway" /></label><div className="locked-note span-2"><span className="status">Plan owner</span><div><strong>You</strong><small>Assigned automatically from the signed-in account.</small></div></div><label>Weekly frequency*<input name="weeklyFrequency" type="number" min="1" max="14" step="1" inputMode="numeric" required placeholder="e.g. 4" /><small>Number of sessions per week</small></label><label>Start date*<input name="startDate" type="date" required /></label><label>Target date<input name="targetDate" type="date" /></label><label>Next review<input name="nextReviewDate" type="date" /></label><label className="span-2">Primary goal*<textarea name="primaryGoal" rows={3} required placeholder="The measurable outcome this plan is working toward" /></label><label className="span-2">Precautions<textarea name="precautions" rows={3} placeholder="Loading restrictions, red flags, and performance constraints" /></label></div><div className="form-note"><span>4</span> SOPCare will create four phases: Protect & restore, Build capacity, Sport integration, and Return to performance.</div><ModalActions busy={busy} primary="Create rehabilitation plan" /></form>;
}
...
function HandoverForm({ injury, practitioners, onSubmit, busy }: { injury: Injury; practitioners: Practitioner[]; onSubmit: (e: FormEvent<HTMLFormElement>) => void; busy: boolean }) {
  return <form onSubmit={onSubmit}><ModalHeading kicker="Clinical handover" title="Endorse care" text={`Transfer responsibility for ${injury.athleteName} without losing the clinical context.`} /><div className="form-grid one-column"><label>Receiving practitioner*<select name="toUserId" required><option value="">Choose practitioner</option>{practitioners.map((person) => <option key={person.userId} value={person.userId}>{person.name} · {person.specialty}{person.clinicCity ? ` · ${person.clinicCity}` : ""}</option>)}</select></label><label>Clinical handover summary*<textarea name="summary" rows={6} required placeholder="Current status, completed care, restrictions, risks, and the next required action." /></label></div><ModalActions busy={busy} primary="Send handover request" /></form>;
}

function TaskHandoverForm({ task, busy, onAccept, onDecline }: { task: ClinicalTask; busy: boolean; onAccept: () => void; onDecline: () => void }) {
  return <div><ModalHeading kicker="Approval required" title={task.title} text="Review the transferred clinical responsibility before responding." /><div className="locked-note"><span className="status">Handover</span><div><strong>Clinical summary</strong><small>{task.detail}</small></div></div><div className="modal-actions"><button className="button secondary" type="button" disabled={busy} onClick={onDecline}>Decline</button><button className="button primary" type="button" disabled={busy} onClick={onAccept}>{busy ? "Saving…" : "Accept care"}</button></div></div>;
}

function ReturnToPlayForm({ task, onSubmit, busy }: { task: ClinicalTask; onSubmit: (e: FormEvent<HTMLFormElement>) => void; busy: boolean }) {
  return <form onSubmit={onSubmit}><ModalHeading kicker="Medical clearance" title="Return-to-play decision" text={task.detail} /><div className="form-grid one-column"><label>Medical decision*<select name="decision" required defaultValue=""><option value="">Select decision</option><option>Authorized — full return</option><option>Authorized with restrictions</option><option>Further medical review required</option><option>Not authorized</option></select></label><label>Restrictions<textarea name="restrictions" rows={3} placeholder="Required limitations, staged exposure, or monitoring instructions." /></label><label>Medical rationale*<textarea name="note" rows={5} required placeholder="Document the evidence and rationale supporting this decision." /></label></div><ModalActions busy={busy} primary="Record medical decision" /></form>;
}
