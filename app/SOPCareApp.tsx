...
function EncounterForm({ actor, athlete, onSubmit, busy }: { actor: Bootstrap["actor"]; athlete: Athlete; onSubmit: (e: FormEvent<HTMLFormElement>) => void; busy: boolean }) {
  return <form onSubmit={onSubmit}><ModalHeading kicker="Clinical encounter" title="New encounter" text={`Record a new clinical encounter for ${fullName(athlete)}.`} /><div className="encounter-author-strip"><Avatar name={actor.name} size="sm" /><span><small>Practitioner</small><strong>{actor.name}</strong><p>{actor.specialty}</p></span><b>Verified account</b></div><input type="hidden" name="athleteId" value={athlete.id} /><section className="encounter-context"><div><small>Athlete</small><strong>{fullName(athlete)}</strong><span>{athlete.mrn}</span></div><div><small>Encounter</small><strong>{actor.defaultEncounterType}</strong><span>{actor.clinicType} · {actor.clinicCity}</span></div></section><div className="form-grid"><label className="span-2">Reason for visit*<input name="reason" required autoFocus placeholder="Concise clinical reason" /></label><label className="span-2 history-field">History<RichHistoryInput /></label><label className="span-2">Diagnosis*<textarea name="diagnosis" rows={3} required placeholder="Write the clinical diagnosis in plain language — no ICD-10 code required" /></label><label className="span-2">Visibility<select name="visibility"><option>Care team</option><option>Restricted</option></select></label></div><div className="modal-actions"><button className="button primary" type="submit" disabled={busy}>{busy ? "Saving…" : "Save encounter"}</button></div></form>;
}
