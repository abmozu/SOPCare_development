...
type RehabilitationMeasurementKind = "rom" | "special_test" | "functional_test";
type RehabilitationMeasurementDraft = { id: string; type: RehabilitationMeasurementKind; label: string; side: string; mode: string; value: string; unit: string; result: string; notes: string };
...
export default function SOPCareApp({ identity, onSwitchWorkspace, onLogout }: { identity: PortalUser; onSwitchWorkspace?: () => void; onLogout: () => void }) {
  ...
  const canCreateClinical = data.actor.permissions.includes("clinical.notes.create");
  const canEditClinical = data.actor.permissions.includes("clinical.notes.edit");
  const canEditAthletes = data.actor.permissions.includes("athletes.edit");
  ...
  return (
    <div className="app-shell">
      ...
      {modal && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setModal(null); }}>
        <section className={`modal ${modal === "encounter" || modal === "injury" || modal === "rehabilitation" || modal === "rehabSession" ? "modal-wide" : ""}`} role="dialog" aria-modal="true" aria-labelledby="modal-title">
          ...
          {modal === "rehabilitation" && <RehabilitationForm injuries={data.injuries} encounters={data.encounters} actor={data.actor} activePlanInjuryIds={data.rehabilitationPlans.filter((plan) => plan.status === "Active").map((plan) => plan.injuryId)} selectedInjuryId={selectedInjury?.id} onSubmit={submitRehabilitation} busy={busy} />}
          {modal === "rehabSession" && selectedRehabilitation && <RehabilitationSessionForm plan={selectedRehabilitation} phases={data.rehabilitationPhases} exercises={data.rehabilitationExercises} sessions={data.rehabilitationSessions.filter((session) => session.planId === selectedRehabilitation.id)} measurements={data.rehabilitationMeasurements.filter((measurement) => measurement.planId === selectedRehabilitation.id)} onSubmit={submitRehabSession} busy={busy} />}
...
function RehabilitationView({ plans, measurements, stats, onNew, onOpen }: { plans: RehabilitationPlan[]; measurements: RehabilitationMeasurement[]; stats: Bootstrap["stats"]; onNew?: () => void; onOpen: (id: string) => void }) {
  ...
  return <>
    <PageHeading eyebrow="Rehabilitation workspace" title="Plans & progression" text="Turn clinical decisions into measurable phases, sessions, and criteria-led progression." action={onNew ? <button className="button primary" onClick={onNew}>＋ Create rehabilitation plan</button> : undefined} />
...
function EncountersView({ encounters, athletes, initialAthleteId, initialEncounterId, embedded = false, onNew, onAthlete: _onAthlete, onManageInjury, onInjury, onSave }: { encounters: Encounter[]; athletes: Athlete[]; initialAthleteId?: string; initialEncounterId?: string | null; embedded?: boolean; onNew?: () => void; onAthlete: (id: string) => void; onManageInjury?: (encounterId: string) => void; onInjury?: (injuryId: string, encounterId: string) => void; onSave: (id: string, fields: EncounterUpdate) => Promise<boolean> }) {
  ...
  return <>
    {!embedded && <PageHeading eyebrow="Unified longitudinal record" title="Medical File" text="Select any athlete, then review every specialty visit in one chronological record." action={onNew ? <button className="button primary" onClick={onNew}>＋ New encounter</button> : undefined} />}
    <section className="medical-file-shell">
      <div className="medical-file-commandbar">
        ...
        <div className="medical-file-actions">{embedded && onNew && <button className="button primary small" onClick={onNew}>＋ New encounter</button>}</div>
      </div>
...
const rehabilitationPhaseDefaults = [
  { title: "Protect & restore", goal: "Control symptoms and restore confident movement", exitCriteria: "Symptoms controlled and foundational movement restored" },
  { title: "Build capacity", goal: "Restore strength, endurance, and tissue load tolerance", exitCriteria: "Objective capacity criteria met with stable response" },
  { title: "Sport integration", goal: "Reintroduce progressive sport-specific demand", exitCriteria: "Sport-specific loading completed without adverse response" },
  { title: "Return to performance", goal: "Restore full training and competition readiness", exitCriteria: "Full exposure completed and shared return decision approved" },
];

function RehabilitationForm({ injuries, encounters, actor, activePlanInjuryIds, selectedInjuryId, onSubmit, busy }: { injuries: Injury[]; encounters: Encounter[]; actor: Bootstrap["actor"]; activePlanInjuryIds: string[]; selectedInjuryId?: string; onSubmit: (e: FormEvent<HTMLFormElement>) => void; busy: boolean }) {
  const availableInjuries = injuries.filter((injury) => injury.stage !== "Closed" && !activePlanInjuryIds.includes(injury.id));
  const initialInjuryId = availableInjuries.some((injury) => injury.id === selectedInjuryId) ? selectedInjuryId ?? "" : "";
  const [injuryId, setInjuryId] = useState(initialInjuryId);
  const initialInjury = availableInjuries.find((injury) => injury.id === initialInjuryId);
  const [title, setTitle] = useState(initialInjury ? `${initialInjury.title} rehabilitation` : "");
  const [phases, setPhases] = useState(rehabilitationPhaseDefaults);
  const selectedInjury = availableInjuries.find((injury) => injury.id === injuryId);
  const linkedEncounter = encounters.filter((encounter) => encounter.injuryId === injuryId).sort((a, b) => new Date(b.encounterDate).getTime() - new Date(a.encounterDate).getTime())[0];
  const today = todayInputDate();
  const selectInjury = (value: string) => {
    setInjuryId(value);
    const injury = availableInjuries.find((item) => item.id === value);
    setTitle(injury ? `${injury.title} rehabilitation` : "");
  };
  const updatePhase = (index: number, field: "goal" | "exitCriteria", value: string) => setPhases((current) => current.map((phase, phaseIndex) => phaseIndex === index ? { ...phase, [field]: value } : phase));
  return <form onSubmit={onSubmit} className="rehab-workflow-form"><ModalHeading kicker="Rehabilitation pathway" title="Create rehabilitation plan" text="Build the pathway from the injury record. Athlete, clinical context, owner, role, and city stay automatic." /><div className="workflow-step"><span>1</span><div><strong>Confirm the clinical context</strong><small>Choose an open injury without an active rehabilitation plan.</small></div></div><div className="form-grid"><label className="span-2">Injury episode*<select name="injuryId" required value={injuryId} onChange={(event) => selectInjury(event.target.value)}><option value="">Select open injury</option>{availableInjuries.map((injury) => <option key={injury.id} value={injury.id}>{injury.athleteName} · {injury.title}</option>)}</select></label>{selectedInjury && <section className="rehab-record-context span-2"><div className="rehab-record-context-head"><div><small>Linked clinical record</small><strong>{selectedInjury.athleteName}</strong><span>{selectedInjury.mrn} · {selectedInjury.sport}</span></div><Status value={selectedInjury.stage} /></div><div className="rehab-record-facts"><span><small>Injury</small><b>{selectedInjury.title}</b></span><span><small>Area</small><b>{selectedInjury.bodyArea} · {selectedInjury.laterality}</b></span><span><small>Onset</small><b>{shortDate(selectedInjury.onsetDate)}</b></span><span><small>Severity</small><b>{selectedInjury.severity}</b></span></div>{linkedEncounter && <div className="rehab-record-baseline"><small>Latest linked encounter</small><strong>{linkedEncounter.diagnosis || linkedEncounter.reason}</strong><p>{linkedEncounter.reason}</p></div>}</section>}</div><div className="workflow-step"><span>2</span><div><strong>Set the working plan</strong><small>Only rehabilitation-specific decisions need entry.</small></div></div><div className="form-grid"><label className="span-2">Plan title*<input name="title" required value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Generated from the injury; edit if needed" /></label><label>Weekly frequency*<input name="weeklyFrequency" type="number" min="1" max="14" step="1" inputMode="numeric" required placeholder="e.g. 4" /><small>Sessions per week</small></label><label>Start date*<input name="startDate" type="date" defaultValue={today} required /></label><label>Target date<input name="targetDate" type="date" min={today} /></label><label className="span-2">Primary goal*<textarea name="primaryGoal" rows={3} required placeholder="A measurable outcome, e.g. complete full training at competition intensity without symptom increase" /></label><div className="locked-note span-2"><span className="status">Plan owner</span><div><strong>{actor.name}</strong><small>{actor.specialty} · {actor.clinicCity}</small></div></div><details className="clinical-checkin-details span-2"><summary><span><strong>Optional plan details</strong><small>Add only when clinically useful</small></span><b>＋</b></summary><div className="form-grid"><label>Next review<input name="nextReviewDate" type="date" min={today} /></label><label className="span-2">Precautions<textarea name="precautions" rows={3} placeholder="Loading restrictions, red flags, and performance constraints" /></label></div></details></div><div className="workflow-step"><span>3</span><div><strong>Review the four phases</strong><small>Defaults are ready; adjust only the goal or exit criteria that differ for this athlete.</small></div></div><div className="phase-builder">{phases.map((phase, index) => <details key={phase.title} open={index === 0}><summary><span>{index + 1}</span><div><strong>{phase.title}</strong><small>{phase.goal}</small></div><b>⌄</b></summary><div className="phase-builder-fields"><label>Phase goal<textarea rows={2} value={phase.goal} onChange={(event) => updatePhase(index, "goal", event.target.value)} /></label><label>Exit criteria<textarea rows={2} value={phase.exitCriteria} onChange={(event) => updatePhase(index, "exitCriteria", event.target.value)} /></label></div></details>)}</div><input type="hidden" name="phases" value={JSON.stringify(phases)} /><ModalActions busy={busy} primary="Create rehabilitation plan" /></form>;
}

function measurementDraft(type: RehabilitationMeasurementKind, source?: RehabilitationMeasurement): RehabilitationMeasurementDraft {
  return { id: crypto.randomUUID(), type, label: source?.label ?? "", side: "", mode: type === "rom" ? (source?.context.includes("PROM") ? "PROM" : "AROM") : "", value: "", unit: type === "rom" ? "°" : source?.unit ?? "", result: "", notes: "" };
}

function RehabilitationSessionForm({ plan, phases, exercises, sessions, measurements, onSubmit, busy }: { plan: RehabilitationPlan; phases: RehabilitationPhase[]; exercises: RehabilitationExercise[]; sessions: RehabilitationSession[]; measurements: RehabilitationMeasurement[]; onSubmit: (e: FormEvent<HTMLFormElement>) => void; busy: boolean }) {
  const defaultSessionDate = new Date(Date.now() - new Date().getTimezoneOffset() * 60_000).toISOString().slice(0, 16);
  const sessionTypes = ["Gym rehabilitation", "Field rehabilitation", "Pool rehabilitation", "Manual therapy", "Assessment", "Other"];
  const [sessionType, setSessionType] = useState(sessionTypes[0]);
  const [customSessionType, setCustomSessionType] = useState("");
  const [measurementRows, setMeasurementRows] = useState<RehabilitationMeasurementDraft[]>([]);
  const currentPhase = phases.find((phase) => phase.id === plan.currentPhaseId);
  const currentExercises = exercises.filter((exercise) => exercise.phaseId === currentPhase?.id);
  const latestSession = [...sessions].sort((a, b) => new Date(b.sessionDate).getTime() - new Date(a.sessionDate).getTime())[0];
  const previousMeasurements = latestSession ? measurements.filter((measurement) => measurement.sessionId === latestSession.id && ["rom", "special_test", "functional_test"].includes(measurement.metricType)) : [];
  const addMeasurement = (type: RehabilitationMeasurementKind) => setMeasurementRows((current) => [...current, measurementDraft(type)]);
  const updateMeasurement = (id: string, field: keyof RehabilitationMeasurementDraft, value: string) => setMeasurementRows((current) => current.map((item) => item.id === id ? { ...item, [field]: value } : item));
  const removeMeasurement = (id: string) => setMeasurementRows((current) => current.filter((item) => item.id !== id));
  const copyPreviousDefinitions = () => {
    const seen = new Set<string>();
    const copied = previousMeasurements.filter((measurement) => {
      const key = `${measurement.metricType}:${measurement.label}`;
      if (seen.has(key)) return false;
      seen.add(key);
      return true;
    }).map((measurement) => measurementDraft(measurement.metricType as RehabilitationMeasurementKind, measurement));
    setMeasurementRows((current) => [...current, ...copied]);
  };
  const resolvedSessionType = sessionType === "Other" ? customSessionType : sessionType;
  return <form onSubmit={onSubmit} className="rehab-session-form"><ModalHeading kicker={`Phase ${plan.currentPhase} · ${plan.currentPhaseTitle}`} title={`Log session ${sessions.length + 1}`} text="Record today’s work first, then add only the measurements that matter." /><section className="session-auto-context"><div><small>Athlete</small><strong>{plan.athleteName}</strong></div><div><small>Injury</small><strong>{plan.injuryTitle}</strong></div><div><small>Phase</small><strong>{plan.currentPhaseTitle}</strong></div><div><small>Session</small><strong>#{sessions.length + 1}</strong></div></section><div className="workflow-step"><span>1</span><div><strong>Session essentials</strong><small>Date, practitioner, athlete, injury, phase, and session number are automatic.</small></div></div><div className="form-grid"><label>Session date & time*<input name="sessionDate" type="datetime-local" defaultValue={defaultSessionDate} max={defaultSessionDate} required /></label><label>Phase progress · 0–100*<input name="phaseProgress" type="number" min="0" max="100" defaultValue={plan.currentPhaseProgress} required /><small>Previous: {plan.currentPhaseProgress}%</small></label><fieldset className="span-2 session-type-field"><legend>Session type*</legend><div className="session-type-picker">{sessionTypes.map((type) => <button type="button" key={type} className={sessionType === type ? "active" : ""} onClick={() => setSessionType(type)}>{type}</button>)}</div>{sessionType === "Other" && <input value={customSessionType} onChange={(event) => setCustomSessionType(event.target.value)} placeholder="Enter session type" required autoFocus />}</fieldset><input type="hidden" name="sessionType" value={resolvedSessionType} /><input type="hidden" name="status" value="Completed" /></div>{currentExercises.length > 0 && <section className="session-prescription"><div><small>Current prescription</small><strong>Use this as the session checklist</strong></div><div>{currentExercises.map((exercise) => <span key={exercise.id}><b>{exercise.name}</b><small>{exercise.dosage} · {exercise.target}</small></span>)}</div></section>}<div className="workflow-step"><span>2</span><div><strong>Clinical session note</strong><small>Four focused entries replace one long unstructured paragraph.</small></div></div><div className="form-grid one-column session-note-grid"><label>Athlete report today*<textarea name="athleteReport" rows={3} required placeholder="Symptoms, aggravating factors, flare-ups, current status, and adherence since the last session" /></label><label>Treatment completed*<textarea name="treatmentCompleted" rows={4} required placeholder="Exercises, dosage, manual treatment, field or pool work, and any changes from the prescription" /></label><label>Response & next action*<textarea name="responseAndNextAction" rows={3} required placeholder="Clinical response, assessment, progression decision, and what happens next" /></label><label>Additional session notes<textarea name="additionalNotes" rows={3} placeholder="Optional observations or multidisciplinary notes that do not belong to a specific measurement" /></label></div><div className="form-grid session-quick-markers"><label>Load score · 0–10<input name="loadScore" type="number" min="0" max="10" /></label><label>Pain before · 0–10<input name="painPre" type="number" min="0" max="10" /></label><label>Pain after · 0–10<input name="painPost" type="number" min="0" max="10" /></label><label>Compliance<select name="compliance" defaultValue=""><option value="">Not recorded</option><option>Compliant</option><option>Partially compliant</option><option>Not completed</option></select></label></div><div className="workflow-step"><span>3</span><div><strong>Add today’s measurements</strong><small>Add as many ROM movements, special tests, or functional tests as needed.</small></div></div><section className="measurement-builder"><div className="measurement-builder-actions"><button type="button" onClick={() => addMeasurement("rom")}>＋ ROM</button><button type="button" onClick={() => addMeasurement("special_test")}>＋ Special test</button><button type="button" onClick={() => addMeasurement("functional_test")}>＋ Functional test</button>{previousMeasurements.length > 0 && <button type="button" className="copy-previous" onClick={copyPreviousDefinitions}>↻ Use previous test names</button>}</div>{measurementRows.length === 0 ? <div className="measurement-empty"><strong>No detailed measurements added</strong><p>That is fine for a routine session. Add only what was measured today.</p></div> : <div className="measurement-draft-list">{measurementRows.map((measurement, index) => <RehabilitationMeasurementEditor key={measurement.id} measurement={measurement} index={index} onChange={updateMeasurement} onRemove={removeMeasurement} />)}</div>}</section><input type="hidden" name="measurementRows" value={JSON.stringify(measurementRows)} /><ModalActions busy={busy} primary="Save session" /></form>;
}

function RehabilitationMeasurementEditor({ measurement, index, onChange, onRemove }: { measurement: RehabilitationMeasurementDraft; index: number; onChange: (id: string, field: keyof RehabilitationMeasurementDraft, value: string) => void; onRemove: (id: string) => void }) {
  const titles = { rom: "ROM measurement", special_test: "Special test", functional_test: "Functional test" };
  return <article className={`measurement-draft measurement-${measurement.type}`}><header><div><span>{index + 1}</span><strong>{titles[measurement.type]}</strong></div><button type="button" onClick={() => onRemove(measurement.id)} aria-label={`Remove ${titles[measurement.type]}`}>× Remove</button></header><div className="form-grid">{measurement.type === "rom" && <><label>Joint / movement*<input value={measurement.label} onChange={(event) => onChange(measurement.id, "label", event.target.value)} placeholder="e.g. Knee flexion" required /></label><label>Side<select value={measurement.side} onChange={(event) => onChange(measurement.id, "side", event.target.value)}><option value="">Not applicable</option><option>Right</option><option>Left</option><option>Bilateral</option></select></label><label>ROM type<select value={measurement.mode} onChange={(event) => onChange(measurement.id, "mode", event.target.value)}><option>AROM</option><option>PROM</option></select></label><label>Result · degrees*<input type="number" min="-30" max="220" step="0.1" value={measurement.value} onChange={(event) => onChange(measurement.id, "value", event.target.value)} required /></label></>}{measurement.type === "special_test" && <><label>Test name*<input value={measurement.label} onChange={(event) => onChange(measurement.id, "label", event.target.value)} placeholder="e.g. Lachman test" required /></label><label>Side<select value={measurement.side} onChange={(event) => onChange(measurement.id, "side", event.target.value)}><option value="">Not applicable</option><option>Right</option><option>Left</option><option>Bilateral</option></select></label><label className="span-2">Result*<select value={measurement.result} onChange={(event) => onChange(measurement.id, "result", event.target.value)} required><option value="">Select result</option><option>Negative</option><option>Positive</option><option>Inconclusive</option><option>Not tolerated</option></select></label></>}{measurement.type === "functional_test" && <><label>Test name*<input value={measurement.label} onChange={(event) => onChange(measurement.id, "label", event.target.value)} placeholder="e.g. Single-leg hop" required /></label><label>Side<select value={measurement.side} onChange={(event) => onChange(measurement.id, "side", event.target.value)}><option value="">Not applicable</option><option>Right</option><option>Left</option><option>Bilateral</option></select></label><label>Result*<input value={measurement.value} onChange={(event) => onChange(measurement.id, "value", event.target.value)} placeholder="e.g. 142 or Passed" required /></label><label>Unit<input value={measurement.unit} onChange={(event) => onChange(measurement.id, "unit", event.target.value)} placeholder="seconds, cm, reps, %" /></label></>}<label className="span-2">Measurement note<textarea rows={2} value={measurement.notes} onChange={(event) => onChange(measurement.id, "notes", event.target.value)} placeholder="Optional: pain, quality, compensation, end-feel, or testing conditions" /></label></div></article>;
}
