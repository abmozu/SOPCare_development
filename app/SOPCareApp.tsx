...
function RichHistoryInput() {
  ...
  return <div className="rich-history-input"><div className="history-toolbar" role="toolbar" aria-label="History formatting"><button type="button" aria-label="Bold" onClick={() => format("bold")}><b>B</b></button><button type="button" aria-label="Italic" onClick={() => format("italic")}><i>I</i></button><button type="button" aria-label="Underline" onClick={() => format("underline")}><u>U</u></button><button type="button" onClick={() => format("insertUnorderedList")}>• List</button><input aria-label="Text colour" type="color" defaultValue="#006c46" onChange={(event) => format("foreColor", event.target.value)} /></div><div ref={editorRef} className="history-content" contentEditable suppressContentEditableWarning data-placeholder="Record symptoms, assessment, decisions, treatment and return-to-sport actions. You can paste formatted text here." onInput={(event) => setValue(event.currentTarget.innerHTML)} /><input type="hidden" name="plan" value={value} /></div>;
}
