    ...
    <div className="app-shell">
      ...
      <div className="main-shell">
        ...
        <main className="content">
          ...
          {view === "Profile" && selected && <ProfileView athlete={selected} athletes={data.athletes} encounters={data.encounters} injuries={data.injuries.filter((injury) => injury.athleteId === selected.id)} practitioners={data.practitioners} tab={profileTab} setTab={setProfileTab} onBack={() => navigate("Athletes")} onAthlete={showProfile} onEncounter={() => setModal("encounter")} onManageInjury={(encounterId) => { setPendingEncounterId(encounterId); setModal("injuryAssociation"); }} onInjury={showInjury} onEdit={() => setModal("edit")} onCare={() => setModal("care")} onSafety={editClinicalSafety} onSave={saveEncounterFields} />}
        </main>
      </div>
...
function ProfileView({ athlete, athletes, encounters, injuries, practitioners, tab, setTab, onBack, onAthlete, onEncounter, onManageInjury, onInjury, onEdit, onCare, onSafety, onSave }: { athlete: Athlete; athletes: Athlete[]; encounters: Encounter[]; injuries: Injury[]; practitioners: Practitioner[]; tab: string; setTab: (v: string) => void; onBack: () => void; onAthlete: (id: string) => void; onEncounter: () => void; onManageInjury: (encounterId: string) => void; onInjury: (id: string) => void; onEdit: () => void; onCare: () => void; onSafety: (category: "allergies" | "chronicConditions" | "prohibitedMedications") => void; onSave: (id: string, fields: EncounterUpdate) => Promise<boolean> }) {
  ...
  return <>
    ...
    {tab.startsWith("Encounters") && <><ClinicalSafetyPanel athlete={athlete} onEdit={onSafety} /><EncountersView encounters={encounters} athletes={athletes} initialAthleteId={athlete.id} embedded onNew={onEncounter} onAthlete={onAthlete} onManageInjury={onManageInjury} onInjury={onInjury} onSave={onSave} /></>}
