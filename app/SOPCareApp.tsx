    ...
    <div className="app-shell">
      ...
      {modal && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setModal(null); }}>
        <section className={`modal ${modal === "encounter" || modal === "injury" || modal === "rehabilitation" ? "modal-wide" : ""}`} role="dialog" aria-modal="true" aria-labelledby="modal-title">
          ...
          {modal === "rehabSession" && selectedRehabilitation && <RehabilitationSessionForm plan={selectedRehabilitation} milestones={data.rehabilitationExercises.filter((item) => item.phaseId === selectedRehabilitation.currentPhaseId)} onSubmit={submitRehabSession} busy={busy} />}
...
function RehabilitationDetailView({ plan, phases, exercises, sessions, onBack, onInjury, onAthlete, onSession, onAdvance }: { plan: RehabilitationPlan; phases: RehabilitationPhase[]; exercises: RehabilitationExercise[]; sessions: RehabilitationSession[]; onBack: () => void; onInjury: (id: string) => void; onAthlete: (id: string) => void; onSession: () => void; onAdvance: () => void }) {
  ...
  const canAdvance = plan.status === "Active" && (current?.progress ?? 0) >= 100;
  return <>
...
function RehabilitationSessionForm({ plan, milestones, onSubmit, busy }: { plan: RehabilitationPlan; milestones: RehabilitationExercise[]; onSubmit: (e: FormEvent<HTMLFormElement>) => void; busy: boolean }) {
  return <form onSubmit={onSubmit}><ModalHeading kicker={`Phase ${plan.currentPhase} · ${plan.currentPhaseTitle}`} title="Record rehabilitation session" text={`Capture delivery, response, and the next action for ${plan.athleteName}.`} /><div className="form-grid"><label>Session date & time*<input name="sessionDate" type="datetime-local" required /></label><label>Session status*<select name="status" defaultValue="Completed"><option>Completed</option><option>Scheduled</option></select></label><label className="span-2">Session type*<input name="sessionType" required placeholder="e.g. Field + gym rehabilitation" /></label><label>Load score · 0–10<input name="loadScore" type="number" min="0" max="10" /></label><label>Pain before · 0–10<input name="painPre" type="number" min="0" max="10" /></label><label>Pain after · 0–10<input name="painPost" type="number" min="0" max="10" /></label>{milestones.length > 0 && <fieldset className="span-2"><legend>Milestones achieved</legend>{milestones.map((item) => <label key={item.id}><input type="checkbox" name="milestoneIds" value={item.id} defaultChecked={item.status === "Complete"} /> {item.name}</label>)}</fieldset>}<label className="span-2">Session notes<textarea name="notes" rows={4} placeholder="What was completed and how did the athlete respond?" /></label><label className="span-2">Next action*<textarea name="nextAction" rows={3} required placeholder="The next clear rehabilitation action" /></label></div><div className="form-note">Progress is calculated automatically from completed milestones.</div><ModalActions busy={busy} primary="Record session" /></form>;
}
