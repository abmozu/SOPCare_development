      ...
      <div className={`medical-file-body ${directoryOpen ? "directory-open" : ""}`}>
        ...
        <div className="medical-record-stage">
          ...
          <div className="medical-file-workspace medical-file-two-column">
            <aside className="visit-history"><div className="medical-column-head"><div><span className="section-kicker">One timeline</span><h3>{filteredEncounters.length} encounters</h3></div><span className="chronology-mark">Newest first</span></div><div className="visit-history-list">{filteredEncounters.map((encounter) => { const date = new Date(encounter.encounterDate); return <button key={encounter.id} className={selectedEncounter?.id === encounter.id ? "active" : ""} onClick={() => setSelectedEncounterId(encounter.id)}><span className="visit-date"><strong>{String(date.getDate()).padStart(2, "0")}</strong><small>{new Intl.DateTimeFormat("en-GB", { month: "short" }).format(date)}<br />{date.getFullYear()}</small><time>{makkahTime(encounter.encounterDate)}</time></span><span className="visit-card-copy"><i>{initials(encounter.practitioner)}</i><span><strong>{encounter.practitioner}</strong><small>{encounter.specialty}</small><p>{encounter.clinicType}<br />{encounter.clinicLocation} · {encounter.clinicCity}</p></span></span></button>; })}{!filteredEncounters.length && <div className="medical-empty"><span>⌕</span><strong>No matching visits</strong><p>Clear the filters or choose another athlete.</p></div>}</div></aside>
            <main className="consultation-panel">{selectedEncounter && selectedAthlete ? <><div className="medical-column-head"><div><span className="section-kicker">Visit review</span><h3>{selectedEncounter.encounterType}</h3></div><button className="button secondary small pdf-button" onClick={() => window.print()}>↓ Download PDF</button></div><VisitReviewEditor encounter={selectedEncounter} onSave={onSave} /></> : <div className="medical-empty large"><span>≡</span><strong>Select an encounter</strong><p>The visit review will appear here.</p></div>}</main>
          </div>
        </div>
      </div>
    </section>
...
function VisitReviewEditor({ encounter, onSave }: { encounter: Encounter; onSave: (id: string, fields: EncounterUpdate) => Promise<boolean> }) {
  ...
  const fields: Array<{ field: EditableEncounterField; letter: string; title: string; empty: string }> = [
    { field: "subjective", letter: "S", title: "Athlete report & sport context", empty: "No subjective history recorded." },
    { field: "objective", letter: "O", title: "Examination & performance measures", empty: "No objective findings recorded." },
    { field: "assessment", letter: "A", title: "Clinical impression", empty: "No assessment recorded." },
    { field: "plan", letter: "P", title: "Treatment & return-to-sport plan", empty: "No management plan recorded." },
  ];
  ...
  return <><div className="visit-completion-bar"><div><span className={`completion-dot ${amendmentMode ? "amending" : ""}`} /><strong>{amendmentMode ? "Amendment in progress" : "Visit completed"}</strong><small>{amendmentMode ? `Reason: ${amendmentReason}` : "The clinical record is locked until an amendment is started."}</small></div>{encounter.canEdit === 1 && !amendmentMode && <button className="button secondary small" onClick={() => setRequestingAmendment(true)}>✎ Amend visit</button>}{amendmentMode && <button className="button primary small" disabled={editing !== null} onClick={() => { setAmendmentMode(false); setSaveState("Visit completed"); }}>Finish amendment</button>}</div>{requestingAmendment && <div className="amendment-request"><label>Reason for amendment<input autoFocus value={amendmentReason} onChange={(event) => setAmendmentReason(event.target.value)} placeholder="Explain why this completed visit needs a change" /></label><div><button className="button secondary small" onClick={() => setRequestingAmendment(false)}>Cancel</button><button className="button primary small" disabled={!amendmentReason.trim()} onClick={beginAmendment}>Start amendment</button></div></div>}<section className="editable-diagnosis"><div><span className="section-kicker">Diagnosis</span><span className="free-text-chip">Free text</span></div>{editing === "diagnosis" ? editor("diagnosis", "Diagnosis") : <p>{values.diagnosis || "No diagnosis recorded."}</p>}{encounter.canEdit === 1 && amendmentMode && editing !== "diagnosis" && <button aria-label="Edit diagnosis" onClick={() => setEditing("diagnosis")}>✎</button>}</section><div className="consultation-meta visit-location-meta"><span><small>Clinic</small>{encounter.clinicType}</span><span><small>Location</small>{encounter.clinicCity}</span></div><section className="chief-complaint"><span>Reason for visit / presenting concern</span><p>{encounter.reason}</p></section>{encounter.visibility === "Restricted" ? <div className="restricted-medical-file"><span>◉</span><h3>Restricted specialty note</h3><p>The clinical narrative is visible only to authorized specialty roles.</p></div> : <div className="sports-soap-history editable-soap">{fields.map(({ field, letter, title, empty }) => <section key={field}><span>{letter}</span><div><h4>{title}</h4>{editing === field ? editor(field, title) : <p>{values[field] || empty}</p>}</div>{encounter.canEdit === 1 && amendmentMode && editing !== field && <button aria-label={`Edit ${title}`} onClick={() => setEditing(field)}>✎</button>}</section>)}</div>}<div className={`autosave-state ${saveState.startsWith("Save failed") ? "error" : ""}`}><span>●</span>{saveState}{savedAt && ` at ${savedAt}`}{encounter.canEdit !== 1 && " · Read-only visit"}</div></>;
}
